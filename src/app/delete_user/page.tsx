...
export default function DeleteUser() {
    const { users, deleteUser, getUser, error, setErrorNull } = useContext(UserContext)
                ...
                { users.map((user) => {
                    return (
                        <div key={user.id} className={ styles.container__users }>
                            ...
                            <button onClick={() => handleDeleteUser(user.id as number)} className={ styles_delete_user.button__delete } >Delete</button>
                        </div>
                    )
                }) }
